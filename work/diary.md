...
SqDistSchedulerを作成。
・距離はユークリッド距離の二乗
・未工事でまだ選らんでいない辺のうち、すでに選らんだ辺との最小距離が最大
で選んでいく方法。
...
パワポで課題を整理しつつ辺削除時の挙動をNetworkX + plotlyで可視化。
seed=7427(最小サイズのグラフ)でe=0とかだと削除してもそれほど影響はでないが、e=786だと
その辺自体は短いが迂回路が長く、その影響がグラフ全体に広がる。

2辺の組合せによって「相性」があるので調査。

ちらっとTwitterをみたらなんとかえでさんがAtCoderに入社している！びっくり。

2辺の組合せを可視化してみているとやはり迂回路をさらに工事するのは良くなくて大幅なコスト増につながることがある。
面白いのはコストが減るケースで局所的に密になっているケースで、互いの迂回路には入っていない AND 迂回路が重複していると
コストが減る。

イメージ的には局所的にk辺連結な構造があればそこはまとめて消しても大丈夫ということ。

# 2023/02/02

行き帰りで考えたのはk辺連結(k>=3)は構造として制約が強い(平面グラフでN=3だと1通りしかない)。
3辺連結でなくても部分的に2辺消しても大丈夫な構造はあるのでもう少し緩く探してもOK。

コミュニティ抽出の最短路木に何回含まれる(edge betweenness)か多い順に削っていくやり方が実装は楽そう。
より直接的に2辺以上同時に消しても大丈夫な構造を探した方がさらに良さそうだがNext stepか。

可視化してみたがコミュニティ抽出だと2辺を同時に消せるような集合になっていないことが多そう。
edge betweennessによるコミュニティ抽出だと良く使われる最短路は削除されて個別に扱われるが
コストが減るケースは良く使われる最短路を含む閉路＋バイパスがあるケースなのでやりたいこととずれていることが明白に。

やはり閉路検出してバイパスがあるものを探すことになりそう。
ただ、閉路＋バイパスに対してD日以内に連結性を保った計画を作れるのか見極めが難しい。

恐らく、
・閉路で分割
・各閉路の辺数のうち他の閉路との共有辺を除いた辺数の最大値
が必要な日数になっている。

昨日、2時くらいまでやったせいか頭がぼーっとして余り回っていない。

# 2023/02/03

昨日は1時くらいに寝た。

閉路を列挙しようかと思ったが面の列挙ができれば良いので双対グラフが構築できれば良い。
ただ、探してみたものの意外と？双対グラフの構築は見つからなくてようやく見つかったものを拝借。

可視化をしてみて面集合を選んでその中でEdge betweennessの高いもの同士を同時に消すのがよさそうなことを確認。
あとEdge betweennessより距離も考慮した方が良いかと思っていたが、単純な回数の積み上げの方がよさそうなことに可視化して気づいた。
確かにその辺を削除した時の全体への影響度合いは回数 x (迂回路の長さ - 元の長さ)で回数の方が効いているのはその通りな気がする。

面の単位で考えると辺が通常は2回カウントされるのでどうするのかを考えて、優先順位をつけて先に決めたものは最悪、隣の面内での
迂回路を使ってもらうことにして除外して考えることにした。

面、面の集合を管理するクラスを書いて、面集合を作っていくところの途中で1:30になったので明日への影響を考えて終了。
今回は平日の水-金をほぼ可視化、考察にあててほとんどコーディングできなかったがこれがどう出るか。

# 2023/02/04

順位表をみたらPsyhoさんの50Gが崩れてbowwowforeachさんが一位に。スゴイ。
相対スコア自体はあまり変わっていないので紙一重の差で争っている。

FaceGroup生成を実装。FaceのMax Betweennessが高いものから隣接するFaceの中でMax Betweennessの高いものを貪欲にくっつけていく。
ただ、あまり大きな領域を作ると「迂回路が小さい」という条件が成立しなくなるので適当に制約を付けた。

バグりながらも可視化した結果と突合しながら進めて想定した結果を出力するようになったのが夕方。

ここからFaceGroupSchedulerを実装。最初、貪欲に決めていったがそれでやると末端で非連結な成分が出てしまう。
結局、全域木を復活させたがコスト増という結果に。

迂回路が残っているとまずいのか？ということでBypassSetSchedulerにまで戻してそこから迂回路がない時にmax betweennessを最小化
するようにしてみたがコスト増に…。うーん。

1つはコスト関数がおかしい。同一日に同一Faceから工事するとダメでコスト(※)を付けているがそれが頻発している。
原理的には非選択辺については起きないはずなのに起きている…。

手で強制的にコストが下がる組合せを入れると最終コストは下がるのでアプローチが悪いわけではない。
いったんバイパスコストは通常0になるのでいったん(※)を外してみる。
探索のコストでも手で辺の工事日を変えてあげるとコストは下がるので探索が良くない。

うーん、ランダム配置からBypassを0にした後にMax Betweennessを最小化してもコスト削減にはつながっていないようだ。
むしろコストアップになるケースも。
うーむ。このタイミングでこれは誤算だな…。

# 2023/02/05

起きてから最後、どうするか悩んだがFaceGroupを1つに絞って、正確なコストを出しながら観察。
アイディア自体は悪くない…はずでFaceGroupを詰めることに。

夕方にようやく動いた。(Kがシビアなやつは制約が充足できないのでConnector+BypassSetを使うことに。3/2000問なので影響はすくない)
遷移を後半はBetweennessの大きいものを中心にして少し改善。

あとはひたすら焼きなまし法のチューニング。
結局、山登り法になった…。

最後の最後でコスト推定で固定点からの距離も入れてみたが改善せず。

最終提出のつもりコードの実行時間がテスト用の2sのままになっていた危ない。

暫定順位は175位でフィニッシュ。
TLを見てて上位陣との差分は

・コスト関数を真面目に計算すると時間がかかる
⇒4～15点くらいで近似
⇒これは最後にやったけど計算時間上限だと悪化。

・最短路は最短路木を持って再計算が必要かどうかを判定し高速化
⇒初期状態は同じ考えをしていたが、探索途中の場合はできていなかった。
辺を削除する場合はそれで高速化できるが、辺を追加する場合にどう高速化するかはちょっと分かっていない。

・長時間回した結果だとパスが選ばれることが多くなる
⇒パスを優先して作る

# 2023/02/10

ymatsuxさんのツイートを参考に最短路木の差分計算を入れて9点サンプリングで評価したら平均コスト9.9M、
相対スコア40.4Gまで伸びた！
高速化勝負だったかぁ。

あと山登り法で工事日を変える際に隣接する辺と同じにするのを優先する…でさらに平均コスト9.6M、相対スコア
42.3Gまで伸びた。

# 2023/02/11

Add/DelEdgeで変更した場所を持っておいて復元するのは逆に遅くなった。

辺追加を先にして削除していってコストが変更前を越えたら打ち切るのを入れるとさらに1.6倍くらい速くなった。
平均コスト9.6M、相対スコア42.5Gまで伸びたが大分とサチってきたので復習は終了。
...
* 連結性(01/29)
  ...
  * ランダムに全域木を作っているが最短路木をベースに作った方がよさそう
    * そもそも距離の総和は差分計算でO(M), うまくやるとO(log N)くらいで出来るのでそれを最小化する全域木を求めれば良さそう
